...
for i in $(seq 1 12); do
  ...
  if [ -n "$RESP" ]; then
    ...
    if [ "$STATUS" = "ok" ] && [ "$PB_CONN" = "true" ] && [ "$VERSION" = "$EXPECTED_VERSION" ]; then
      ...
      info "Status: $STATUS | PB connected: $PB_CONN | Version: $VERSION"
      exit 0
    fi
    info "Attempt $i: status=$STATUS pb=$PB_CONN version=$VERSION"
  else
